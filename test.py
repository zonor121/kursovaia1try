...
class GraphVisualizerTkinter:
    def __init__(self):
        ...
        # Холст для рисования
        self.canvas = tk.Canvas(self.root, width=1000, height=600, bg=self.current_theme['canvas_bg'], 
        ...
        self.theme_btn = ttk.Button(self.control_frame, text="🌙 Тёмная тема", command=self.toggle_theme)
    ...
    def apply_theme(self):
        ...
        # Обновляем фоны всех фреймов
        self.update_widget_colors(self.control_frame)
        self.update_widget_colors(self.selection_frame)
        self.update_widget_colors(self.speed_frame)
        self.update_widget_colors(self.results_frame)
    ...
    def draw_graph(self):
        ...
        if not self.graph:
            text_color = "white" if self.dark_mode else "gray"
    ...
    def draw_edges(self):
        ...
        for (u, v), weight in self.graph.items():
            if u in self.positions and v in self.positions:
                ...
                edge_color = "lightgray" if not self.dark_mode else "#555555"
    ...
    def draw_edge_weight(self, x1, y1, x2, y2, weight):
        ...
        weight_color = "red" if weight < 0 else ("white" if self.dark_mode else "darkblue")
    ...
    def draw_nodes(self):
        ...
        for node, (orig_x, orig_y) in self.positions.items():
            ...
            self.canvas.create_oval(
                ...
                x + node_radius, y + node_radius,
                fill=fill_color, outline="white" if self.dark_mode else "black", width=max(1, int(2 * self.zoom_level))
            )
    ...
    def draw_node_distance(self, x, y, node):
        ...
        distance_color = "white" if self.dark_mode else "darkred"
        self.canvas.create_text(
            x + 30 * self.zoom_level, y - 30 * self.zoom_level,
            text=dist_text, fill=distance_color,
